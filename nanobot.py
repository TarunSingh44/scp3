...
class Node:
    def __init__(self, host, port, marker, name):
        ...
        self.host = host
        self.port = port
        logging.info(f"Node {self.name} initialized at {self.host}:{self.port}")
    ...
    def set_actuator(self, actuator, value):
     try:
        if actuator == 'tethers':
            ...
            if 'beacon' in self.__actuators:  # Only for the primary bot.
                # Activate beacon (send a data packet to the Rendezvous server
                # marking the position of the primary bot that has detected the
                ...
                # (Send an interest packet to the Rendezvous server
                # marking the turn off of the beacon by this particular node.)
                else:  # value == 0
     ...
     try:
        if sensor == 'cancer_marker':
            ...
            if value == 0 and self.marker == CONFIG['primary_marker']:
                ...
                # Update the value in content store with the latest marker value detected.
...
if __name__ == '__main__':
    try:
        ...
        logger.info(f"Bot '{args.name}' started with host {args.host}, port {args.port}, and marker {args.marker}")
    except Exception as e:
